...
<section class="print-hideen border-bottom" style="padding: 5px 15px;background:#364a60;">
    <div class="row pl-2">
        <div class="col-md-6 pl-1">
            <h3 style="font-family:Cambria;font-size: 2rem;color:white; float: left;">Sales Revenue</h3>
        </div>
        <div class="col-md-6">
                        ...
                        <table class="table table-bordered table-sm" data-table="sales_revinew" id="invoice">
                            <thead class="thead">
                                <tr>
                                    ...
                                    <th style="text-align: center !important; min-width:fit-content">Taxable
                                        Amount <br>
                                        {{ number_format($grandTotal->sum('budget'), 2) }}

                                    </th>
                                    <th style="text-align: center !important; min-width:fit-content">VAT <br>
                                        {{ number_format($grandTotal->sum('vat'), 2) }}

                                    </th>
                                    <th style="text-align: center !important; min-width:fit-content">Retention <br>
                                        {{ number_format($grandTotal->sum('retention_amount'), 2) }}

                                    </th>
                                    <th style="text-align: center !important; min-width:fit-content">Total Amount <br>
                                        {{ number_format($grandTotal->sum('vat')  + $grandTotal->sum('budget'), 2) }}
                            ...
                            <tbody id="sale-body-hide">
                                ...
                                $receipt_sales = \App\ReceiptSale::where('sale_id', $item->id)->get();

                                foreach($receipt_sales as $receipt_sale){
                                $receipt_voucher = \App\Receipt::find($receipt_sale->payment_id);
                                $payment += $receipt_voucher->total_amount;
                                }

                                $item->paid_amount = $payment;
                                $item->due_amount = $item->total_budget - ($payment +
                                ...
                                <tr class="sale_view text-uppercase home-sale-view" id="{{$item->id}}">
                                    ...
                                    <td style="text-align: center !important;"
                                        title="{{$item->company->company_name??'SBBC'}}">
                                        {{\Illuminate\Support\Str::limit($item->company->company_name??'SBBC',15)}}
                                    </td>
                                    ...
                                    <td style="text-align: right !important;">
                                        ...
                                        <td style="text-align: right !important;">
                                            {{number_format($item->vat,2)}}</td>

                                             <td style="text-align: right !important;">
                                        {{number_format($item->retention_amount>0?$item->retention_amount:0,2)}}</td>
                                ...
                                {{-- <tr style=" background-color: #3d4a94 !important;">
                                    <td colspan="10" style="text-align: right ; margin-right:5px; color:#fff;">Total</td>
                                    <td style="color:#fff;">{{ number_format($grandTotal, 2) }}</td>
                                </tr> --}}
