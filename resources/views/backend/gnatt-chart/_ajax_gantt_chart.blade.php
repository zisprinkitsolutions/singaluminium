...
<section class="print-hideen border-bottom" style="background: #364a60;">
    ...
    @endif --}}
        ...
        <div class="pr-1 w-100 pl-2" style="margin-top: 2px;">
            <h4 style="font-family:Cambria;font-size: 2rem;color:white;"> Gantt Chart (Project:
                {{ $project->project_name }} , Project No: {{ $project->prospect->project_no }} , Plot:
                {{ $project->prospect->plot }} , Location: {{ $project->prospect->location }}) </h4>
        </div>
    </div>
</section>
    ...
    <nav>
        <div class="d-flex justify-content-between align-items-center mb-2">
            <div class="nav nav-tabs nav-tabs1" id="nav-tab" role="tablist">
                @if (!$chart_have)
                    <button class="nav-link nav-link1 active" id="nav-create-tab"data-project="{{ $project->id }}"
                        data-toggle="tab" data-target="#nav-create" type="button" role="tab"
                        aria-controls="nav-create" aria-selected="true">
                        🛠️ Build Gantt Chart
                    </button>
                @endif

                <button class="nav-link nav-link1 " id="nav-tracking-tab" data-project="{{ $project->id }}"
                    data-toggle="tab" data-target="#nav-tracking" type="button" role="tab"
                    aria-controls="nav-tracking" aria-selected="false"
                    @if (!$chart_have) disabled @endif>
                    📊 Tracking
                </button>

                <button class="nav-link nav-link1 " id="nav-edit-tab" data-project="{{ $project->id }}"
                    data-toggle="tab" data-target="#nav-edit" type="button" role="tab" aria-controls="nav-edit"
                    aria-selected="false" @if (!$chart_have) disabled @endif>
                    ✏️ Edit
                </button>

                <button class="nav-link nav-link1 " id="nav-view-tab" data-project="{{ $project->id }}"
                    data-toggle="tab" data-target="#nav-view" type="button" role="tab" aria-controls="nav-view"
                    aria-selected="false" @if (!$chart_have) disabled @endif>
                    👁️ View
                </button>
                ...
                <button type="button" class="nav-link1 gantt-chart-button-delete gantt-chart-button"
                    data-action="delete" data-url="{{ route('gnatt.chart.destroy', $project->id ?? 0) }}">
                    🗑️ Delete
                </button>
            </div>
    ...
    <div class="tab-content" id="nav-tabContent">
        <div class="tab-pane fade show active" id="nav-create" role="tabpanel" aria-labelledby="nav-create-tab">
            ...
            <section class="p-1" id="widgets-Statistics">
                ...
                <form class="repeater mt-1 project-form chart-form" action="{{ route('gnatt.chart.store') }}"
