...
<div class="app-content content print-hideen">
    ...
    <div class="content-wrapper">
        <div class="content-body">
            <form action="{{route('master-account-final-excel-import')}}" method="POST">
                @csrf
                <div class="table-responsive">
                    <table class="table mb-0 table-sm table-hover table-bordered">
                        <thead  class="" style="position:sticky; background: #1a233a;">
                            <tr style="height: 30px;">
                                <th>Master A/C Head <span class="text-danger">*</span></th>
                                <th>Master Account Details <span class="text-danger">*</span></th>
                                <th>Definition <span class="text-danger">*</span></th>
                                <th>Master A/C Type <span class="text-danger">*</span></th>
                                <th>VAT Type <span class="text-danger">*</span></th>
                                <th>Action</th>
                            </tr>
                        </thead>
                        @php
                            ...
                            @foreach ($records as $key => $t_record)
                                <tr class="trFontSize" id="{{'tr'.$t_record->id}}">
                                    ...
                                    <td class="semi-mini-width">
                                        <input name="fld_ac_head[]" style="width: 100% !important;" type="text" value="{{$t_record->fld_ac_head}}" required class="tr_value_submit {{$t_record->fld_ac_head?'':'error'}}" title="Head name is required">
                                    </td>
                                    <td class="semi-mini-width">
                                        <select name="account_type[]" class="common-select2 inputFieldHeight tr_value_submit" style="width: 100% !important" id="account_type" required>
                                            <option value="">Select...</option>
                                            @foreach ($categories as $item)
                                            <option value="{{ $item->title }}" {{$t_record->account_type==$item->title?'selected':''}}>{{ $item->title }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    <td class="semi-mini-width">
                                        <select name="definition[]" class="common-select2 inputFieldHeight tr_value_submit" style="width: 100% !important" id="" required>
                                            <option value="">Select...</option>
                                            @foreach ($mst_definitions as $item)
                                            <option value="{{ $item->title }}" {{$t_record->definition==$item->title?'selected':''}}>{{ $item->title }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    <td class="semi-mini-width">
                                        <select name="ac_type[]" class="common-select2 inputFieldHeight tr_value_submit" style="width: 100% !important" id="mst_ac_type" {{ isset($masterAcc)? "disabled readonly":"required"}} >
                                            <option value="">Select...</option>
                                            @foreach ($mstAccType as $item)
                                            <option value="{{ $item->title }}" {{$t_record->ac_type==$item->title?'selected':''}}>{{ $item->title }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    <td class="semi-mini-width">
                                        <select name="vat_type[]" id="vat_type" class="common-select2 inputFieldHeight tr_value_submit" style="width: 100% !important" required>
                                            <option value="">Select..</option>
                                            @foreach ($vat_types as $item)
                                            <option value="{{ $item->title }}" {{$t_record->vat_type==$item->title?'selected':''}}>{{ $item->title }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    <td>
...
@push('js')
    <script>
        $(document).on('click','.excel-import-delete', function(e){
            ...
            $.ajax({
                method: "post",
                url: "{{route('master-account-delete-excel')}}",
                data: {
                    id:id,
                    _token: _token,
                },
        ...
        $(document).on("change", ".tr_value_submit", function(){
            ...
            if(!field_value){
                ...
                $.ajax({
                    method: "post",
                    url: "{{route('update-master-account')}}",
                    data: {
