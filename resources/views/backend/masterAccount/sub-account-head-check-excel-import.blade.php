...
<div class="app-content content print-hideen">
    ...
    <div class="content-wrapper">
        <div class="content-body">
            <form action="{{route('sub-account-head-final-excel-import')}}" method="POST">
                @csrf
                <div class="">
                    <table class="table mb-0 table-sm table-hover table-bordered">
                        <thead  class="" style="position:sticky; background: #1a233a;">
                            <tr style="height: 30px;">
                                <th>Sub Account Head <span class="text-danger">*</span></th>
                            ...
                            @foreach ($records as $key => $t_record)
                                <tr class="trFontSize" id="{{'tr'.$t_record->id}}">
                                    ...
                                    <td class="semi-mini-width">
                                        <input name="sub_head[]" style="width: 100% !important;" type="text" value="{{$t_record->sub_head}}" required class="tr_value_submit {{$t_record->sub_head?'':'error'}}" title="Head name is required">
                                    </td>
                                    <td class="semi-mini-width">
                                        <select name="account_head[]" class="common-select2 inputFieldHeight tr_value_submit" style="width: 100% !important" id="master_account" required>
                                            <option value="">Select...</option>
                                            @foreach ($account_head as $item)
                                            <option value="{{ $item->fld_ac_head }}" {{$t_record->account_head==$item->fld_ac_head?'selected':''}}>{{ $item->fld_ac_head }}</option>
                                            @endforeach
                                        </select>
                                    </td>
    ...
    <script>
        ...
        $(document).on("change", ".tr_value_submit", function(){
            ...
            if(!field_value){
                ...
                $.ajax({
                    method: "post",
                    url: "{{route('update-sub-account-head')}}",
                    data: {
