            ...
            <div class="tab-content bg-white">
                <div class="tab-pane active">
                    <div class="d-flex justify-content-between align-items-center" style="max-width: 800px">
                        ...
                        {{-- <a href="{{route('sub-account-export')}}"
                            class="btn btn-outline-secondary mExcelButton  nav-item nav-link ml-1" role="tab"
                            aria-controls="nav-contact" aria-selected="false">
                            <div> Excel Export </div>
                        </a> --}}

                        {{-- <a href="{{ route('sub-account-export') }}" class="btn btn-xs mExcelButton formButton"
                            title="Export to Excel"><img src="{{ asset('/icon/excel-icon.png') }}" class="img-fluid"
                                width="30">Excel Export</a> --}}
                    </div>
                    <hr>
                                ...
                                <div class="card mb-0">
                                    ...
                                    <div id="collapse{{ $accountHead->id }}" class="collapse multi-collapse">
                                        ...
                                        <div class="rowStyle d-flex ml-2" id="tr{{ $item->id }}">
                                            ...
                                            <div>
                                                <a href="#" id="update{{ $item->id }}">{{ $item->name }}</a>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('sub-account-head-excel-import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-1">
...
<script>
        ...
        $(document).on('click', '#add-subhead', function() {
            subheadCount++;
            let isUnit = $(this).data('unit');

            let newInput = `
                <div class="col-md-6 subhead-group d-flex align-items-end gap-1">
                    <div class="w-100">
                        <label>Subhead ${subheadCount}</label>
                        <input type="text" name="name[${subheadCount}]" class="form-control inputFieldHeight">
                    </div>
            ...
            if (isUnit) {
                ...
                newInput += `
                    <div class="col-md-6 subhead-group d-flex align-items-end gap-1">
                        <div class="w-100">
                            <label>Unit</label>
                            <select name="unit_id[${subheadCount}]" class="form-control inputFieldHeight" required>
                                ${unitOptions}
                            </select>
                        </div>
                    </div>
                                ...
                                success: function(response) {
                                    if (response == 1) {
                                        ...
                                        Swal.fire({
                                            icon: 'warning',
                                            title: 'warning!',
                                            text: 'This account sub head already used',
                                            confirmButtonColor: '#3085d6'
                                        });
                                    }
                                }
