<form action="{{route('expense-allocation.store')}}" method="POST" id="project_expense_store">
    @csrf
    ...
    <div class="row">
        <div class="col-md-6">
            <input type="text" required value="{{ Carbon\Carbon::now()->format('d/m/Y') }}" class="form-control inputFieldHeight datepicker" name="date" placeholder="dd/mm/yyyy">
        </div>
    </div>
    <table class="table table-sm ">
        ...
        <tbody id="task_TBody">
            ...
            <tr id="task_TRow" class="text-center invoice_row">
                ...
                <td>
                    <select name="task_id[]" id="task_id" class="w-100 task_id inputFieldHeight2"></select>
                </td>
                <td>
                    <select name="task_item_id[]" id="task_item" class="w-100 task_item inputFieldHeight2"></select>
                </td>
                <td>
                    <div>
        ...
        <tbody>
            <tr>
                <td class="text-right pr-1" colspan="3" style="color: black">TOTAL AMOUNT <small class="text-danger">(Max amount allow: {{$expense->amount - $expense->out_amount}} And QTY: {{$expense->qty - $expense->out_qty}}  )</small></td>
                <td>
                    <input type="number" step="any" readonly id="task_total_qty" class="text-center inputFieldHeight2 form-control inputFieldHeight task_total_qty" name="task_total_amount" value="" placeholder="QTY" >
                </td>
                <td>
                    <input type="number" step="any" readonly id="task_total_amount" class="text-center inputFieldHeight2 form-control inputFieldHeight task_total_amount" name="task_total_amount" value="" placeholder="TOTAL " >
                </td>
            </tr>
        </tbody>
    </table>
