    ...
    <div class="content-wrapper">
        <div class="content-body">
            ...
            <div class="tab-content bg-white">
                <div id="journalList" class="tab-pane active pl-1 pr-1">
                    <input type="hidden" name="standard_vat_rate" value="{{$standard_vat_rate}}"  id="standard_vat_rate">
                    <section id="widgets-Statistics">
                        @isset($journalF)
                        <form action="{{ route('journalEntryEditPost', $journalF) }}" method="POST" enctype="multipart/form-data" id="form_submit">
                        @else
                                ...
                                <div class="card-body">
                                    <div class="row d-flex justify-content-center" style="">
                                        <div class="col-md-3 changeColStyle  col-right-padding d-none">
                                            ...
                                            <select name="transection_type" class="common-select2 w-100" id="transection_type">
                                        ...
                                        <div class="col-md-3 form-group d-none col-right-padding d-none">
                                            <label for="">Journal Entry No</label>
                                            <input type="text" class="form-control" id="journal_no" value="{{$journalF->journal_no }}" name="journal_no" placeholder="Journal Entry No" readonly>
                                            @error('journal_no')
                                                <div class="btn btn-sm btn-danger">{{ $message }}
                                                </div>
                                            @enderror
                                        ...
                                        <div class="col-md-6 transaction_type d-none">
                                    ...
                                    <div class="cardStyleChange " style="width: 100%">
                                        <div class="card-body bg-white " style="padding: 0px !important;">
                                            <table class="table table-bordered table-sm mb-0">
                                                ...
                                                @endphp
                                                <tbody id="TBody">
                                                    @foreach($journalF->records->where('is_main_head',1) as $index => $item)
                                                    <tr>
                                                        <td>
                                                             <select name="group-a[{{$index}}][multi_acc_head]" onchange="option(this);"  class="text-center job_group_id form-control multi-acc-head input-due-payment inputFieldHeight2 account_head_change" style="width: 100%;    HEIGHT: 36PX;" required>
                                                                <option value="">Select...</option>
                                                                @foreach ($acHeads as $head)
                                                                <option value="{{ $head->id }}"{{ $item->account_head_id == $head->id ? 'selected' : ''}}> {{ $head->fld_ac_head }}</option>
                                                                @foreach ($head->sub_heads as $sub_head)
                                                                    <option value="Sub{{$sub_head->id}}" class="sub-head" {{$item->sub_account_head_id==$sub_head->id?'selected':''}}> &nbsp;&nbsp;&nbsp; |---{{$sub_head->name}}</option>
                                                                @endforeach
                                                                @endforeach
                                                            </select>
                                                        </td>
                                                        <td><input type="text" name="group-a[{{$index}}][description]" value="{{$item->description}}" class="text-center form-control inputFieldHeight2 "style="width: 100%;height:36px;"></td>
                                                        <td><input type="number" name="group-a[{{$index}}][debit_amount]" step="any" {{$item->transaction_type=='DR'?'':'readonly'}} value="{{$item->transaction_type=='DR'?$item->amount:null}}" class="text-center form-control inputFieldHeight2 feild_amount debit_amount" style="width: 100%;height:36px;"></td>
                                                        <td><input type="number" name="group-a[{{$index}}][credit_amount]" step="any" {{$item->transaction_type=='CR'?'':'readonly'}} value="{{$item->transaction_type=='CR'?$item->amount:null}}" class="text-center form-control inputFieldHeight2 feild_amount credit_amount" style="width: 100%;height:36px;"></td>
                                                ...
                                                <tbody>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <input type="text" readonly id="total_debit" value="{{$journalF->records->where('is_main_head',1)->where('transaction_type', 'DR')->sum('amount')}}" class="text-center form-control inputFieldHeight2 @error('total_debit') error @enderror inputFieldHeight total"
                                                        ...
                                                        <td>
                                                            <input type="text" readonly id="total_credit" value="{{$journalF->records->where('is_main_head',1)->where('transaction_type', 'CR')->sum('amount')}}" class="text-center form-control inputFieldHeight2 @error('total_credit') error @enderror inputFieldHeight total"
                            ...
                            <div class="cardStyleChange">
                                <div class="card-body bg-white">
                                    <div class="row">
                                        ...
                                        <div class="col-md-12 text-right d-flex justify-content-center align-items-center mt-1" >
...
<script>
    ...
    function BtnAdd(trow, tbody, inputs) {
        ...
        newRow.find('.vat-exist').find('.empty_field').val(5)
