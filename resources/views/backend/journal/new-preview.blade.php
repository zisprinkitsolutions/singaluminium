...
<section class="print-hideen border-bottom" style="padding: 5px 20px;background: #34465b;">
    <div class="d-flex flex-row-reverse align-items-center" style="">
        <div class="" style=""><a href="#" class="close btn-icon btn btn-danger" data-dismiss="modal"
                aria-label="Close"><span aria-hidden="true"><i class='bx bx-x' style="padding: 3px 0px;"></i></span></a>
        </div>
        {{-- <div class="" style="padding-right: 3px;"><a href="#" id="{{ $journal->id }}"
                onclick="window.print();" class="btn btn-icon btn-success" title="Print"><i
...
<section id="widgets-Statistics">
                                ...
                                @foreach ($journal->records()->orderBy('transaction_type', 'DESC')->get() as $record)
                                    <tr class="text-center trFontSize">
                                        <td class="text-left pl-1">
                                            {{ $record->ac_sub_head ? $record->ac_sub_head->name : $record->account_head }}
                                        </td>
                                        <td>{{ $record->invoice_no }}</td>

                                        <td class="text-right pr-1">
                                            {{ $retVal = $record->transaction_type == 'DR' ? $currency->symbole . ' ' . number_format($record->amount, 2) : '' }}
                                        </td>
                                        <td class="text-right pr-1">
                                            {{ $retVal = $record->transaction_type == 'CR' ? $currency->symbole . ' ' . number_format($record->amount, 2) : '' }}
                                        </td>
                                    </tr>
                                @endforeach
    ...
    <div class="row pt-2">
        @if ($journal->voucher_scan != '' && $journal->voucher_scan2 != '')
            ...
            <div class="col-6 text-center">
                <img src="{{ asset('storage/upload/documents') }}/{{ $journal->voucher_scan }}" class="img-fluid"
                    style="height: 490px" alt="">
            </div>
            <div class="col-6 text-center">
                <img src="{{ asset('storage/upload/documents2') }}/{{ $journal->voucher_scan2 }}" class="img-fluid"
                    style="height: 490px" alt="">
            </div>
    ...
    <div class="d-flex flex-row-reverse justify-content-center align-items-center pb-2" style="">
        <div class="print-hideen" style="">
            <a href="#" id="{{ $journal->id }}" onclick="window.print();" class="btn btn-icon btn-secondary custom-action-btn" title="Print Now">
                <i class='bx bx-printer'></i> Print
            </a>
        </div>
        <div class="print-hideen">
            <a href="{{ route('journal-view-pdf', $journal->id) }}" class="btn btn-icon btn-primary custom-action-btn" title="PDF"><i
