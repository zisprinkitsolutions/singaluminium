<thead>
    <tr class="parrent-tr">
        ...
        <td style="font-size: 10px; font-weight:800;  background-color:#e3e3e3; width:10%;"> Paymode</td>
    ...
    @foreach ($records as $record)
        ...
        @if(!$journal->receipt_id && !$journal->payment_id)
            ...
            <tr class="text-center trFontSize " v-type="main" style="cursor: pointer;" id="{{ $journal->journal_id }}">
                    ...
                    <td style="font-size: 10px; text-transform:uppercase;">{{$journal->pay_mode}} </td>
                    @if ($journal->pay_mode == 'Credit')
                        <td style="text-align:right !important; ">0.00</td>
                        <td style="text-align:right !important; ">{{number_format($c=$journal->records()->whereNotIn('account_head_id',[407,1759,1760])->where('transaction_type','DR')->sum('amount'),2)}}</td>
                    @else
                        @php
                            $c=0;
                        @endphp
                        <td style="text-align:right !important; ">{{number_format($journal->records()->whereNotIn('account_head_id',[407,1760,1759])->where('transaction_type','DR')->sum('amount'),2)}}</td>
                        <td style="text-align:right !important; ">0.00</td>
                    @endif
                    @php
                        $balance_dr -=  $c;
            ...
            @else
                <tr class="text-center trFontSize " v-type="main" style="cursor: pointer;" id="{{ $journal->journal_id }}">
                    ...
                    <td style="text-align:right !important; ">{{number_format($b=$journal->records()->whereNotIn('account_head_id',[31,1759,1760])->where('transaction_type','DR')->sum('amount'),2)}}</td>
                    <td style="text-align:right !important; ">0.00</td>
                        @php
                            $balance_dr +=  $b;
                        @endphp
    ...
    @if ($journal->payment_id)
        ...
        @if($payment->count())
            <tr class="text-center trFontSize " v-type="main" style="cursor: pointer;" id="{{ $journal->journal_id }}">
                <td>{{date('d/m/Y', strtotime($journal->date))}}</td>
                <td class="text-left">{{$journal->party_journal_description($journal->id)['name']}}</td>
                <td style="text-transform: uppercase !important;">{{$journal->pay_mode}}</td>
    ...
    @if ($party->pi_type != 'Supplier')
        ...
        @if($payment->count())
            <tr class="text-center trFontSize " v-type="main" style="cursor: pointer;" id="{{ $journal->journal_id }}">
                ...
                @else
                    <td style="text-align:right !important; "> {{number_format($amount>0?$amount:0,2)}}</td>
                    @if ($journal->receipt->type == 'advance')
                        <td style="text-align:right !important; ">{{number_format($journal->total_amount,2)}}</td>
                    @else
                        <td style="text-align:right !important; ">{{number_format($amount<0?($amount*(-1)):0,2)}}</td>
                    @endif
