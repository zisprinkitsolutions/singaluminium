...
@section('content')
    ...
    <style>
        ...
        /* Select2 dropdown option text left align */
        .select2-results__option {
            text-align: left !important;
        }

        /* Selected item (inside input) text left align */
        ...
        /* Mobile এ প্রতিটা section আলাদা row, কিন্তু ভেতরের item গুলো পাশে পাশে */
                            ...
                            <div class="col-md-12 pt-2">
                                <div class="cardStyleChange">
                                    <div class="card-body bg-white print-hidden">
                                        <div
                                            class="d-flex flex-wrap justify-content-between align-items-center print-hidden">
                                            <!-- Left Side Buttons -->
                                            <div class="d-flex align-items-center mb-2">
                                                @if (Auth::user()->hasPermission('Expense_Create'))
                                                    <button
                                                        class="btn btn-custom-nis btn-expense inputFieldHeight expense_create_model">
                                                        New Expense
                                                    </button>
                                                @endif

                                                <button
                                                    class="btn btn-custom-nis btn-inventory inputFieldHeight stock_inventory_model">
                                                    Inventory
                                                </button>

                                                <a href="{{ route('project.expense.report') }}"
                                                    class="btn btn-custom-nis btn-summary inputFieldHeight">
                                                    Summary
                                                </a>
                                            </div>
                                            <!-- Center Side (Search + Select + Date) -->
                                            <div class="d-flex align-items-center gap-2 mb-2 justify-content-center">
                                                <input type="text" name="search" id="search"
                                                    class="form-control inputFieldHeight" placeholder="Search by Expense No"
                                                    style="width:500px;"> <!-- এখানে width fix -->

                                                <select name="party_search" id="party_search"
                                                    class="common-select2 inputFieldHeight" style="width:450px;">
                                                    <!-- এখানে width fix -->
                                                ...
                                                <input type="text" name="date_search" id="date_search"
                                                    ...
                                                    placeholder="Search by Date" style="max-width:150px;">
                                            </div>
                                    ...
                                    <table class="table table-bordered table-sm" id="expense">
                                        <thead class="thead">
                                            <tr>
                                                <th style="text-align: center !important; min-width: 50px">SL</th>
                                                <th style="text-align: center !important; min-width:fit-content">Date
                                                </th>
                                                <th style="text-align: center !important; min-width: 90px">Purchase No
                                                </th>
                                                <th style="text-align: left !important; min-width:fit-content">Party
                                                    Name </th>
                                                <th style="text-align: left !important; min-width:fit-content">Amount
                                                    <small>(@if (!empty($currency->symbole))
                                                            {{ $currency->symbole }}
                                                        @endif)</small>
                                                    <br> {{ number_format($data['total_amount'], 2) }}
                                                </th>
                                                <th style="text-align: left !important;min-width: fit-content"> Remarks
                                                </th>
                                                <th style="text-align: left !important; min-width: fit-content">Pay Status
                                                </th>
                                                <th style="text-align: left !important; min-width:fit-content">Payment
                                                    <br> {{ number_format($data['paid_amount'], 2) }}
                                                </th>
                                                <th style="text-align: left !important; min-width:fit-content"> Due
                                                    Payment <br> {{ number_format($data['due_amount'], 2) }} </th>
                                            </tr>
                                        </thead>
                                        <tbody id="purch-body">
                                            @foreach ($temp_expenses as $key => $item)
                                                <tr class="approve_view" id="{{ $item->id }}">
                                                    <td>{{ ($temp_expenses->currentPage() - 1) * $temp_expenses->perPage() + $key + 1 }}
                                                    </td>
                                                    <td style="text-align: center !important;">
                                                        {{ date('d/m/Y', strtotime($item->date)) }}</td>
                                                    <td style="text-align: center !important;">{{ $item->purchase_no }}
                                                    </td>
                                                    ...
                                                    <td style="text-align: left !important;"> {{$item->status}} </td>
                                                    @if ($item->due_amount > 0 and $item->paid_amount > 0)
                                                        <td style="text-align: left !important;"> <span
                                                                class="bg-warning text-white"> Partial Paid </span>
                                                        </td>
                                                    @elseif($item->due_amount <= 0)
                                                        <td style="text-align: left !important;"> <span
                                                                class="bg-success text-white"> Full Paid </span> </td>
                                                    @else
                                                        <td style="text-align: left !important;"> <span
                                                                class="bg-danger text-white"> Payable </span> </td>
                                                        </td>
                                                    @endif
                                                    <td style="text-align: left !important;"> {{ $item->paid_amount }}
                                                    </td>
                                                    <td style="text-align: left !important;"> {{ $item->due_amount }}
                                                    </td>
                                                </tr>
                                            @endforeach
                                            @foreach ($expenses as $key => $item)
                                                <tr class="expense_view" id="{{ $item->id }}">
                                                    ...
                                                    <td style="text-align: left !important;">
                                                        {{ number_format($item->total_amount, 2) }}</td>
                                                    <td style="text-align: left !important;"> Submitted </td>
                                                    @if ($item->due_amount > 0 and $item->paid_amount > 0)
                                                        <td style="text-align: left !important;"> <span
                                                                class="bg-warning text-white"> Partial Paid </span>
                                                        </td>
    ...
    <div class="modal fade" id="balanceAdd" tabindex="-1" role="dialog" aria-labelledby="balanceAddLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-body d-flex">
                    ...
                    <button type="button"
                        class="btn  btn_create mr-1 float-right formButton"style=" background: #1a233a;color:#fff"
                        title="Yes" id="new_balance_add">
                        Yes
                    </button>
                    <button type="button"
                        class="btn btn_create mr-1 float-right formButton btn-danger"style=" background: #1a233a;color:#fff"
                        title="No" data-dismiss="modal" aria-label="Close">
                        NO
                    </button>
                </div>
            </div>
    ...
    <div class="modal fade bd-example-modal-lg" id="inventory_list_model" tabindex="-1" rrole="dialog"
        aria-labelledby="myLargeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header" style="padding: 5px 18px;background:#364a60;">
                    <h5 class="modal-title" id="exampleModalLabel"
                        style="font-family:Cambria;font-size: 2rem;color:white;">Project Inventory Allocation</h5>
    ...
    <div class="modal fade bd-example-modal-lg" id="expense_create_model" tabindex="-1" role="dialog"
        aria-labelledby="expense_create_modelLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
    ...
    <div class="modal fade bd-example-modal-lg" id="stock_inventory_model" tabindex="-1" role="dialog"
        aria-labelledby="stock_inventory_modelLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
    ...
    <div class="modal fade bd-example-modal-lg" id="project_expense_model" tabindex="-1" rrole="dialog"
        aria-labelledby="myLargeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header" style="padding: 5px 18px;background:#364a60;">
    ...
    <div class="modal fade bd-example-modal-lg" id="subsidiary_model" tabindex="-1" rrole="dialog"
        aria-labelledby="myLargeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header" style="padding: 5px 18px;background:#364a60;">
                    <h5 class="modal-title" id="exampleModalLabel"
                        style="font-family:Cambria;font-size: 2rem;color:white;">Subsidiary Allocation</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
    ...
    <div class="modal fade bd-example-modal-lg" id="check_project_expense_model" tabindex="-1" role="dialog"
        aria-labelledby="check_project_expense_modelLabel" aria-hidden="true" data-backdrop="static"
        data-keyboard="false"> <!-- Prevents closing on click outside or ESC -->
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header" style="padding: 5px 18px;background:#364a60;">
                    <h5 class="modal-title" id="check_project_expense_modelLabel" style="color: #fff">Project Expense
                        base on Account Head</h5>
    ...
    <div class="modal fade bd-example-modal-lg" id="add-new-expense" tabindex="-1" rrole="dialog"
        aria-labelledby="myLargeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                            ...
                            <div class="row">
                                <div class="col-md-12 col-12 mt-1 ">
                                    <div class="form-group">
                                        ...
                                        <select name="findMasterAcc"
                                            class="form-control inputFieldHeight findMasterAcc common-select2"
                                            style="width: 100% !important;">
                                            <option value="">Select-------</option>
                                            @foreach ($special_master_details as $masterAcc)
                                                <option value="{{ route('findMasterAcc', $masterAcc) }}">
                                                    {{ $masterAcc->mst_ac_code }} - {{ $masterAcc->mst_ac_head }}</option>
                                            @endforeach
        ...
        <div class="modal fade" id="rejectModal" tabindex="-1" aria-labelledby="rejectModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="{{ route('purchase-rejected') }}" method="POST" id="modalForm">
                        @csrf
                        <!-- Body -->
                        <div class="modal-body">
                            <!-- Visible Input -->
    ...
    <script>
        ...
        $(document).on('submit', '#cogs_project_expense_update', function(e) {
            ...
            if (Number(task_total_amount) != Number(max_amount)) {
                toastr.warning('The amount must be equal');
            } else {
                $.ajax({
                    url: url,
                    method: 'POST',
        ...
        $(document).on('keyup', '.cogs_task_amount', function(e) {
            var cogs_paid = $(this).val();
            var current_tr = $(this).closest('tr');
            var max_amount = $('#cogs_max_amount').val();
            var max_qty = $('#cogs_max_qty').val();
            var cogs_qty = (cogs_paid / max_amount) * max_qty;
            current_tr.find('.cogs_task_qty').val(cogs_qty.toFixed(2));
            cogs_total();
        });
        ...
        $(document).on('keyup', '.subsidiary_amount', function(e) {
            var subsidiary_amount = $(this).val();
            var current_tr = $(this).closest('tr');
            var max_amount = $('#subsidiary_max_amount').val();
            var max_qty = $('#subsidiary_max_qty').val();
            var subsidiary_qty = (subsidiary_amount / max_amount) * max_qty;
            current_tr.find('.subsidiary_qty').val(subsidiary_qty.toFixed(2));
            subsidiary_total();
        });
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: function(res) {
                                ...
                                Swal.fire(alertDesign('Expense approved successfully',
