<form action="{{route('cogs-project-expense-store')}}" method="POST" id="cogs_project_expense_store">
    ...
    <table class="table table-sm ">
        ...
        <tbody id="task_TBody">
            @foreach ($temp_pe as $temp)
                <tr class="text-center invoice_row">
                    ...
                    <td>
                        <select name="task_id[]" id="task_id" class="w-100 task_id inputFieldHeight2" required>
                            <option value="">Select...</option>
                    ...
                    <td>
                        <select name="task_item_id[]" id="task_item" class="w-100 task_item inputFieldHeight2">
                            <option value="">Select...</option>
                    ...
                    <td>
                        <div>
                            <input type="number" step="any" name="task_amount[]" readonly required value="{{$temp->amount}}" step="any" placeholder="Amount" class="text-center form-control inputFieldHeight2 cogs_task_amount"style="width: 100%;height:36px;" >
                        </div>
                    </td>
            ...
            <tr id="task_TRow" class="text-center invoice_row d-none">
                <td>
                    <select name="group-a[0][project_id]" id="project_id" class="form-control project_id " required disabled>
                        <option value="">Select...</option>
                        @foreach ($projects as $item)
                ...
                <td>
                    <select name="group-a[0][task_id]" id="task_id" class="form-control task_id " disabled required></select>
                </td>
                <td>
                    <select name="group-a[0][task_item_id]" id="task_item" class="form-control task_item" disabled></select>
                </td>
                <td>
                    <div>
                        <input type="number" step="any" name="task_qty[]" required step="any" placeholder="QTY" class="text-center form-control inputFieldHeight2 cogs_task_qty"style="width: 100%;height:36px;">
                    </div>
                </td>
                <td>
                    <div class="d-flex justy-content-between align-items-center">
                        <input type="number" readonly step="any" name="task_amount[]" step="any" placeholder="Amount" class="text-center form-control inputFieldHeight2 cogs_task_amount"style="width: 100%;height:36px;">
                    </div>
                </td>
                <td class="NoPrint text-center">
                    <button style="padding: 5px; margin: 4px;" type="button" class="btn btn-sm btn-danger" onclick="COGS_Btn_Del(this)"><i class="bx bx-trash" style="color: white;margin-top: -5px;"></i></button>
        ...
        <tbody>
            <tr>
                <td class="text-right pr-1 " colspan="3" style="color: black"><small class="text-danger">(Max amount allow: {{$amount}} And QTY: {{$qty}}  )</small>TOTAL:</td>
                <td>
