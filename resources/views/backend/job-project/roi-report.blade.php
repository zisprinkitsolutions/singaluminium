...
<div class="modal-header  print-hideen" style="padding: 5px 15px;background:#364a60;">
    ...
    <h5 class="modal-title" id="exampleModalLabel" style="font-family:Cambria;font-size: 2rem;color:#fff;padding-left: 12px;">
        ROI Report {{optional($project->new_project)->name}}
        <small style="font-size:15px;"> (Project no:{{optional($project->new_project)->project_no . ' , Plot:' . optional($project->new_project)->plot}})</small>
    </h5>

    <div class="d-flex align-items-center">
        {{-- <a href="{{route('new.project.roy.report',['project_id' => $project->id, 'print' => 'print'])}}"
             target="_blank" class="modal-btn bg-info text-white"
             style="margin-right: 0.7rem !important; padding:3px 6px; border:none; border-radius:5px; width:">
            <i class="bx bx-printer" style="margin-top: 5px;"></i>
        </a> --}}

        <button type="button" class="modal-btn bg-danger text-white" data-dismiss="modal" aria-label="Close" style="margin-right: 1.1rem !important; border:none; border-radius:5px;">
          <span aria-hidden="true" style="display: inline-block; font-size: 24px;">&times;</span>
        </button>
                ...
                <div class="left-side">
                    ...
                    <div class="report-table table-responsive">
                        <table class="table table-sm">
                            <thead>
                                <tr>
                                    <th class="text-left pl-1" style="width:30%;"> Project Name </th>
                            ...
                            <tbody>
                                <tr>
                                    <td class="text-left pl-1"> {{$project->project_name}} </td>
                                    {{-- <td class="text-center"> {{optional($project->new_project)->plot}} </td> --}}
                                    <td> {{$project->address}} </td>
                                    <td class="text-left"> {{optional($project->party)->pi_name}} </td>
                        ...
                        <table class="table table-sm">
                            <thead>
                                <tr>
                                    ...
                                    <th style="min-width: 110px; text-align:right !important;"> Matarial Cost  </th>
                                    <th style="width:170px; text-align:right  !important;"> Administrative Cost  </th>
                                    <th style="width:150px; text-align:right  !important;"> Payment </th>
                                    <th style="width:150px; text-align:right  !important;"> Payable </th>
                                </tr>
                            </thead>
                            <tbody>
                                @php
                                    $matarial_cost_1 = (float) str_replace(',', '', $matarial_cost ?? 0);
                                    $labour_cost = (float) str_replace(',', '', $labour_cost ?? 0);
                                    $administrative_cost = (float) str_replace(',', '', $administrative_cost ?? 0);
                                    $total_cost = $matarial_cost_1 + $labour_cost + $administrative_cost;
                                @endphp
                                <tr>
...
<div class="d-flex justify-content-center align-items-center mb-2">
    <a href="{{route('new.project.roy.report',['project_id' => $project->id, 'print' => 'print'])}}"
            target="_blank" class="modal-btn btn btn-secondary custom-action-btn" title="Print Now">
...
<div class="modal fade" id="expenseModal" tabindex="-1"
   aria-labelledby="myLargeModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content" style="margin: 0 25px 0 10px;">
            <div>
                <div class="modal-header  print-hideen" style="padding: 5px 15px;background:#364a60;">
                    <h5 class="modal-title" id="exampleModalLabel" style="font-family:Cambria;font-size:17px;color:#fff;padding-left: 12px;"> Investment & Expense {{number_format($project->purchase_expense->sum('amount'),2)}} <small> ({{$currency->symbole}}) </small> </h5>
                    <div class="d-flex align-items-center">

                        <button type="button" class="close-btn bg-danger text-white" aria-label="Close" style="margin-right: 1.1rem !important; padding:3px 12px;">
                            ...
                            <div class="report-table table-responsive">
                                <table class="table table-sm">
                                    ...
                                    <tbody>
                                        @foreach($payments as $key => $payment)
                                            <tr>
                                                <td class="text-left">{{$payment['party_name']}} </td>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">{{$total_investment}}</td>
                                            <td class="text-center"> {{$total_payment}}</td>
                                            <td class="text-center">{{$total_payable}}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
...
<div class="modal fade" id="revenueModal" tabindex="-1" rrole="dialog"
   aria-labelledby="myLargeModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
      <div class="modal-content" style="margin: 0 25px 0 10px;">
        <div>
            <div class="modal-header  print-hideen" style="padding: 5px 15px;background:#364a60;">
                <h5 class="modal-title" id="exampleModalLabel" style="font-family:Cambria;font-size:17px;color:#fff;padding-left: 12px;"> Recenue {{$sale}} <small> ({{$currency->symbole}}) </small> </h5>
                <div class="d-flex align-items-center">
                        ...
                        <div class="report-table table-responsive">
                            <table class="table table-sm">
                                ...
                                <tbody>
                                    @foreach($project->invoices as $invoice)
                                    <tr>
                                        <td>{{$invoice->party ? $invoice->party->pi_name : 'N/A'}}</td>
                                        <td class="text-center"> {{date('d/m/Y',strtotime($invoice->date))}} </td>
                                        <td class="text-center"> {{$invoice->invoice_no ?? 'N/A' }} </td>
                                        <td class="text-center"> {{$invoice->invoice_type}}</td>
                                        <td class="text-center"> {{number_format($invoice->tasks()->whereNotNull('task_id')->sum('budget'),2)}} </td>
                                        <td class="text-center"> {{number_format($invoice->tasks()->whereNotNull('task_id')->sum('paid_amount'),2)}} </td>
                                        <td class="text-center"> {{number_format($invoice->tasks()->whereNotNull('task_id')->sum('due_amount'),2)}} </td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
