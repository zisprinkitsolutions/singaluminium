            ...
            <div class="tab-content bg-white">
                <div id="journaCreation" class="tab-pane active p-2">
                    <div class="mb-2 d-flex justify-content-between align-item-center print-hideen">
                        ...
                        <form class="form print-hideen mt-1">
                            <div class="form-group d-flex">
                                <select name="customer_id" id="customer_id" class="form-control common-select2" style="width: 200px !important;">
                                    <option value=""> Select Customer </option>
                                    @foreach ($customers as  $item)
                                    <option value="{{$item->id}}" {{isset($customer) ? $customer->id == $item->id ? 'selected' : ' ' : ' '}}> {{$item->pi_name}} </option>
                                    @endforeach
                                </select>
                                <select name="project_id" id="project_id" class="common-select2" style="width: 200px !important;">>
                                ...
                                <div>
                                    <input type="text" class="datepicker form-control inputFieldHeight" placeholder="Date From" name="date_from" style="width: 100px !important;">
                                </div>
                                <div>
                                    <input type="text" class="datepicker form-control inputFieldHeight" placeholder="Date To" name="date_to" style="width: 100px !important;">
                                </div>
                    ...
                    <div>
                        ...
                        <div class="d-flex justify-content-between align-items-center border" style="padding:5px 8px;margin:15px 0;">
                            ...
                            <div>
                                <div class="date d-flex justify-content-end">
                                    <span class="date-title d-block"> Date : </span> <span class="date-value d-block">
                        ...
                        @foreach($project_id?$projects->where('project_id',$project_id):$projects as $work_order)
                            @php
                                $work_order_list = App\JobProject::where('customer_id', $customer->id)->where('project_id', $work_order->project_id)->get();
                            @endphp
                            <div class="project-details mb-1">
                                ...
                                <div class="row">
                                    <div class="col-6 pr-0">
                                        <p style="color:#000;font-weight:400; font-size:15px; margin:0;">
                                            <span style="color:#000;font-weight:500;"> Project Name: </span> {{ Illuminate\Support\Str::limit($work_order->project_id?$work_order->new_project->name:$work_order->project_name,35)}}
                                        </p>
                                    </div>
                                    ...
                                    <div class="col-3">
                                        <p style="color:#000;font-weight:400; font-size:15px; margin:0;">
                                            ...
                                            {{number_format($work_order_list->sum('total_budget')+(($standard_vat_rate/100)*$work_order_list->sum('total_budget')),2,'.','')}}
                                        </p>
                                    </div>
                                    <div class="col-12">
                                        <p style="color:#000;font-weight:400; font-size:15px; margin:0;">
                                            <span style="color:#000;font-weight:500;"> Work Descriptions :
                                                @foreach ($work_order_list as $item)
                                                @if (count($item->invoicess)>0)
                                                {{ Illuminate\Support\Str::limit($item->project_description,100)}},
                                                @endif
                                                @endforeach
                                            </span>
                                        </p>
                                ...
                                {{-- {{ Illuminate\Support\Str::limit($work_order->project_description,200)}} --}}
                            </div>
                                ...
                                <table class="tabl-sm custom-tbl  border-botton  w-100">
                                    ...
                                    <tbody>
                                        @php
                                            ...
                                            $total_due_amount = 0;
                                        @endphp
                                        @foreach ($work_order_list as $project)
                                            @php
                                                $invoices = $project->invoicess;
                                                if($date_from && $date_to){
                                                    $invoices = $invoices->whereBetween('date', [$date_from, $date_to]);
                                                }elseif($date_from){
                                                    $invoices = $invoices->where('date', $date_from);
                                                    // dd($date_from);
                                                }
                                            @endphp
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                $.each(data,function(key,val){
                    $('#project_id').append("<option value='" + val.project_id +"'> " + val.name + " </option>");
                })
            }
        })
    })
