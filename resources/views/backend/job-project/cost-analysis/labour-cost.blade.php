...
<section class="print-hideen border-bottom" style="background: #364a60;">
    <div class="d-flex flex-row-reverse">
        ...
        <div class="pr-1 w-100 pl-2">
            <h4 style="font-family:Cambria;font-size: 2rem;color:white;"> Labour Cost  </h4>
        </div>
    </div>
</section>
...
<section id="widgets-Statistics" style="padding: 15px 22px;">
    <div class="row">
        <div class="col-md-12 text-center invoice-view-wrapper student_profle-print">
            ...
            <h2> Labour Cost Date:{{date('d/m/Y',strtotime($to_date))}} </h2>
            @elseif($from_date && !$to_date)
            <h2> Labour Cost Date:{{date('d/m/Y',strtotime($from_date))}} </h2>
            @elseif ($from_date && $to_date)
            <h2> Labour Cost From: {{date('d/m/Y',strtotime($from_date))}} To: {{date('d/m/Y',strtotime($to_date))}} </h2>
            @else
            <h2> Labour Cost </h2>
            @endif
        </div>
            ...
            <div class="customer-info">
                <table class="table table-sm table-bordered" style="color: #1d1d1d !important;">
                    ...
                    <tr>
                        ...
                        <td class="text-left">
                            <strong style="display: inline-block; width: 100px;"> Labour Cost  </strong>
                            <span style="display: inline-block; width: 10px;">:</span>
                            {{ number_format($total_cost,2) }} AED
                        </td>
                    </tr>
                </table>
            </div>
            ...
            <div class="table-responsive">
                <table class="table table-sm table-bordered border-botton proview-table" style="color: black; ">
                    <thead style="background: #706f6f33 !important;color: black;">
                        <tr>
                            <th class="text-left"  style="text-transform: uppercase; color: black !important;"> Project Name </th>
                            <th class="text-left"  style="text-transform: uppercase; color: black !important;"> Working Days </th>
                            <th style="text-transform: uppercase; color: black !important;"> Working Hours  </th>
                            <th style="text-transform: uppercase; color: black !important;">  Overtime </th>
                            <th class="text-right" style="text-transform: uppercase; color: black !important;"> Late Time  </th>
                            <th class="text-right" style="text-transform: uppercase; color: black !important;"> Absent  </th>
                            <th class="text-right" style="text-transform: uppercase; color: black !important;"> Current Salary  </th>
                            <th class="text-right" style="width:150px; text-transform: uppercase; color: black !important;"> Overtime Amount </th>
                            <th class="text-right" style="text-transform: uppercase; color: black !important;"> Late Penalty </th>
                            <th class="text-right" style="text-transform: uppercase; color: black !important;"> Absent Penalty</th>
                            <th class="text-right" style="width:150px; text-transform: uppercase; color: black !important;"> Total Cost </th>
                        </tr>
                    </thead>

                    <tbody class="user-table-body">
                        @foreach ($data as $key => $item)
                         <tr class="details" data-url="{{route('labour.cost.report.details',[$project->id,$key])}}">
                            <td class="text-left" title="{{$item['employee_name']}}"> {{ \Illuminate\Support\Str::limit($item['employee_name'] ?? 'N/A', $limit = 30, $end = '...')}} </td>
                            <td class="text-center"> {{$item['total_days'] ?? 0 }} </td>
                            <td class="text-center"> {{secondsTotime($item['total_working_hours']) ?? '00:00:00' }} </td>
                            <td class="text-center"> {{secondsTotime($item['total_overtime']) ?? '00:00:00'}} </td>
                            <td class="text-right">{{secondsTotime($item['total_late_time']) ?? '00:00:00'}}</td>
                            <td class="text-right">{{$item['total_absen'] ?? 0 }}</td>
                            <td class="text-right">{{number_format($item['basic_salary_current_day'],2) ?? 0, 2}}</td>
                            <td class="text-right">{{number_format($item['overtime_amount'],2) ?? 0, 2}}</td>
                            <td class="text-right">{{secondsTotime($item['total_late_time']) ?? '00:00:00'}}</td>

                            <td class="text-right">{{number_format($item['total_absen_penalty'],2) ?? 0, 2}}</td>
    ...
    <div class="d-flex flex-row-reverse justify-content-center align-items-center">
        <div class="print-hideen" >
            <a href="#" onclick="window.print();" class="btn btn-icon btn-secondary custom-action-btn mb-1" title="Print Now">
