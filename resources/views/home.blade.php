...
<style>
    ...
    .chart-container-sm {
        position: relative;
        height: 350px !important; /* দুই চার্টের height same */
    }

    /* Card-body ভিতরের padding normalize */
    /* #lineChart-card-body,
    #barChart-card-body {
        padding: 0 !important;
        margin: 0 !important;
    ...
    /* Title / heading এর margin কমানো */
    #lineChart-card-body h6,
    #barChart-card-body h6 {
        margin-bottom: 8px;
        font-weight: 600;
    }

    /* উপরের header অংশের height control */
    .lineChart-header,
    #barChart-card-body > div.d-inline-flex {
        margin-bottom: 5px;
    }
                ...
                <div class="row print-hide ">
                    ...
                    <div class="col-12 p-1">
                        <div class="d-flex flex-wrap gap-2">
                            <input type="text" id="date_from" class="form-control w-auto datepicker"  placeholder="From Date">
                            <input type="text" id="date_to" class="form-control w-auto datepicker " style="margin-left: 3px;" placeholder="To Date">
                            <button id="filterBtn" class="btn btn-primary ml-1 " style="margin-left: 3px;" >Filter</button>
                        </div>
                    </div>

                    {{-- Executive & Approver Section --}}
                    @if ($user->role && in_array(strtolower($user->role->name), ['executive', 'administration']))
                    <div class="col-12 col-md-6">
                        <div class="card shadow-sm border-0 ">
                            <div class="table-head bg-primary text-white d-flex justify-content-between align-items-center">
                                <h5 class="mb-0 text-white">Executive</h5>
                                <a href="#" class="text-white">All <i class="bx bx-right-arrow-alt"></i></a>
                            </div>
                            <div class="card-body p-0">
                                <div class="table-responsive">
                                    <table class="table table-hover table-sm mb-0" id="executiveTable">
                    ...
                    <div class="col-12 col-md-6">
                        <div class="card shadow-sm border-">
                            <div class=" table-head bg-success text-white d-flex justify-content-between align-items-center">
                                <h5 class="mb-0 text-white">Approver Status</h5>
                                <a href="#" class="text-white">All <i class="bx bx-right-arrow-alt"></i></a>
                            </div>
                            <div class="card-body p-0">
                                <div class="table-responsive">
                                    <table class="table table-hover table-sm mb-0" id="approverTable">
                    ...
                    {{-- Authorizer / Approver Section --}}
                    @if ($user->role && in_array(strtolower($user->role->name), ['authorizer', 'approver', 'administration']))
                    <div class="col-12 col-md-6">
                        <div class="card shadow-sm border-0">
                            <div class="table-head bg-warning text-dark d-flex justify-content-between align-items-center">
                                <h5 class="mb-0 text-white">Pendings</h5>
                                <a href="#" class="text-white">All <i class="bx bx-right-arrow-alt"></i></a>
                            </div>
                            <div class="card-body p-0">
                                <div class="table-responsive">
                    ...
                    <div class="col-12 col-md-6">
                        <div class="card shadow-sm border-0">
                            <div class="table-head bg-info text-white d-flex justify-content-between align-items-center">
                                <h5 class="mb-0 text-white">Awaiting Approval</h5>
                                <a href="#" class="text-white">All <i class="bx bx-right-arrow-alt"></i></a>
                            </div>
                            <div class="card-body p-0">
                                <div class="table-responsive">
                    ...
                    {{-- <div class="col-12">
                        <div class="card shadow-sm border-0">
                            <div class="table-head bg-success text-white d-flex justify-content-between align-items-center">
                                <h5 class="mb-0 text-white">My Activity</h5>
                                <a href="#" class="text-white">All <i class="bx bx-right-arrow-alt"></i></a>
                            </div>
                            <div class="card-body p-0">
                                <div class="table-responsive">
                                    <table class="table table-hover table-sm mb-0" id="myActivityTable">
                                        <thead class="table-success text-center">
                                            <tr>
                                                <th>Request ID</th>
                                                <th>Created By</th>
        ...
        <div class="modal-content">
            <div class="modal-header print-hideen" style="padding: 5px 15px;background:#364a60;">
                ...
                <div class="d-flex align-items-center mr-1">
                    ...
                    <button type="button" class="project-btn bg-danger text-white border-0 rounded" style="padding: 5px 12px;" data-dismiss="modal"
                        aria-label="Close">
  ...
  var lineChart = new Chart(ctx1, {
    ...
    data: {
      ...
      datasets: [
        {
          label: 'Total Revenue',
          data: [20, 18, 25, 10, 28, 40, 32, 36, 28, 26, 30, 38],
          borderColor: '#2563eb',
        ...
        {
          label: 'Total Sales',
          data: [25, 28, 30, 22, 40, 45, 36, 38, 32, 30, 35, 40],
          borderColor: '#38bdf8',
  ...
  var barChart = new Chart(ctx2, {
    ...
    data: {
      ...
      datasets: [
        {
          label: 'Sales',
          data: [50, 40, 70, 30, 45, 65, 75],
          backgroundColor: '#2563eb',
          borderRadius: 4
        },
        {
          label: 'Revenue',
          data: [30, 35, 20, 15, 40, 20, 25],
...
{{-- dashboard data show permission wise  --}}
{{-- <script>

    $(document).ready(function () {
        function loadExecutiveTable(dateFrom = '', dateTo = '') {
            $('#executiveTable').DataTable({
                ...
                dataSrc: function (json) {
                    let rows = [];
                    Object.values(json.created_by_data).forEach(function (group) {
                        ...
                        item.status ?? '',
                        item.reject_reason ?? '',
                    `<a href="javascript:void(0);" style="padding:5px 5px;" class="btn btn-sm btn-primary  view-details" data-id="${item.id}"
                            data-source="${item.source}" title="View">
                            View 👁
                        </a>`
                        ]);
                        });
        ...
        function loadApproverTable(dateFrom = '', dateTo = '') {
        $('#approverTable').DataTable({
            ...
            dataSrc: function (json) {
            let rows = [];
            Object.values(json.approved_by_data).forEach(function (group) {
                group.items.forEach(function (item) {
                    rows.push([
                    item.ref_no,
                    item.source,
                    item.status ?? '',
                    item.reject_reason ?? '',
                    `<a href="/details/${item.id}"style="padding:3px 5px;" class="btn btn-sm btn-primary  view-details" data-id="${item.id}"
                        data-source="${item.source}" title="View">
                        View 👁
                    </a>`
                    ]);
        ...
        function loadMyActivityTable(dateFrom = '', dateTo = '') {
            $('#myActivityTable').DataTable({
                ...
                json.data.forEach(function (item) {
                    // Prefer created_by_info, fallback to approved_by_info, then edited_by_info
                        let userInfo = item.created_by_info || item.approved_by_info || item.edited_by_info || {};
                        ...
                        item.status ?? '',
                        item.reject_reason ?? '',
                        `<a href="/details/${item.id}" style="padding:5px 5px;" class="btn btn-sm btn-primary view-details" data-id="${item.id}"
                            data-source="${item.source}" title="View">
                            View 👁
                        </a>`
                        ]);
        ...
        $('#filterBtn').on('click', function () {
            ...
            loadExecutiveTable(dateFrom, dateTo);
            loadApproverTable(dateFrom, dateTo);
            loadMyActivityTable(dateFrom, dateTo);
        });
    });
</script> --}}
@endpush
