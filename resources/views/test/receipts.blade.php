        ...
        <div class="content-body">
            <section id="widgets-Statistics">
                <div class="container pt-2">
                    <button class="btn btn-xs btn-info float-right mr-1" onclick="exportTableToCSV('Sale.csv')">Export
                        To CSV</button>
                    <div class="row d-flex justify-content-center">
                        ...
                        <table class="table-bordered">
                            ...
                            <tr class="">
                                <th>{{ date('m/d/Y', strtotime($receipt->date)) }}</th>
                                <th>{{ $receipt->receipt_no }}</th>

                                <th>{{ $item->invoice->invoice_no }} </th>
                                <th> {{ $item->invoice->project->new_project->project_no }}</th>


                                <th>{{ $item->Total_amount }} </th>
                                <th> {{ $item->invoice->project->new_project->name }}</th>

                                <th>{{ $receipt->party->pi_name }}</th>
                                <th>{{ $receipt->narration }}</th>
                                <th>{{ $receipt->total_amount }}</th>
                                <th>{{$receipt->pay_mode}}</th>
                                <th>{{$receipt->bank_id}}</th>
                            </tr>
                            @endforeach

                            @if ($receipt->items->count() <= 0) <tr class="">
                                <th>{{ date('m/d/Y', strtotime($receipt->date)) }}</th>
                                <th>{{ $receipt->receipt_no }}</th>
