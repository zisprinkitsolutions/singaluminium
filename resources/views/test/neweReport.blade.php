        ...
        <div class="content-body">
            <section id="widgets-Statistics">
                <div class="container pt-2">
                    ...
                    onclick="exportTableToCSV('Sale.csv')">Export To CSV</button>
...
@endphp
                        <table class="table-bordered">
                            <tr>
                                ...
                                <tr class="">
                                    <th>{{ date('m/d/Y',strtotime($invoice->date)) }}</th>
                                    <th>{{ $invoice->invoice_no }}</th>
                                    <th>{{$invoice->retention_invoice? 'Yes':'No'}}</th>
                                    <th> {{$invoice->project->new_project->project_no}}</th>
                                    <th> {{$invoice->project->new_project->name}}</th>
                                    <th>{{ $invoice->party->pi_name }}</th>
                                    @foreach ($invoice->tasks as $item)
                                    <th>{{$item->task_name}}</th>

                                    @endforeach
                                    <th>{{number_format((float)(  $invoice->budget), 2,'.','')   }}</th>
                                    <th>{{number_format((float)(  $invoice->vat), 2,'.','')   }}</th>
                                    <th>{{number_format((float)(  $invoice->retention_amount), 2,'.','')   }}</th>

                                    <th class="{{$invoice->budget+$invoice->vat+$invoice->retention_amount ==$invoice->total_budget ?'bg-secondary':'bg-danger' }}">{{number_format((float)(  $invoice->total_budget), 2,'.','')   }}</th>
                                </tr>
